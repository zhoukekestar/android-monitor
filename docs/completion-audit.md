...
| Simple script entrypoints exist | Daily script usage is now reduced to `scripts/build.sh` and `scripts/install.sh`, documented in `scripts/README.md` and the root `README.md`. `scripts/build.sh` ran successfully on 2026-05-10 and produced `AndroidReceiver/app/build/outputs/apk/debug/android-receiver-debug.apk`. `scripts/install.sh` entered the safe install flow, failed on the current phone with `INSTALL_PARSE_FAILED_INCONSISTENT_CERTIFICATES`, and printed concise keep-existing or `scripts/install.sh --replace` instructions without uninstalling the installed app. `scripts/install.sh --replace` is the explicit replacement path and delegates to the guarded uninstall/reinstall helper. | Build verified; install blocked by existing certificate mismatch |
...
`scripts/display-audit.sh` currently reports one stale Android Monitor virtual
display in this macOS WindowServer session. Start Display and the explicit
